...
void XmlSettings::assignValueV4 ( Vector4DF* buf, std::string key )
{	
	TiXmlElement* elem = (*mCurrBase).ToNode()->FirstChildElement( key );
	if ( elem != 0x0 && elem->GetText() != 0x0 ) {
		*buf = ParseVector4 ( elem->GetText() );	// Assignment only occurs if xml element exists
	}
}
